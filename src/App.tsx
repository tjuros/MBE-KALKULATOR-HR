...
type PackageItem = { weight: string; length: string; width: string; height: string };
...
const DPD_FREE_ISLANDS = new Set([
  ...
  "23250", "53291", "21220", "21223", "23234", "22243",
]);
...
const INTIME_ZONE_3 = new Set([
  ...Array.from(ISLAND_POSTALS),
]);

const GLS_1 = [
  { max: 1, price: 3.1 }, { max: 2, price: 3.1 }, { max: 3, price: 3.3 },
  { max: 5, price: 4.08 }, { max: 10, price: 5.25 }, { max: 15, price: 5.93 },
  { max: 20, price: 7.33 }, { max: 25, price: 7.9 }, { max: 30, price: 9.32 },
  { max: 40, price: 10.52 },
];

const GLS_2_4 = [
  { max: 1, price: 2.34 }, { max: 2, price: 2.34 }, { max: 3, price: 2.51 },
  { max: 5, price: 2.62 }, { max: 10, price: 3.32 }, { max: 15, price: 4.26 },
  { max: 20, price: 4.95 }, { max: 25, price: 5.67 }, { max: 30, price: 6.99 },
  { max: 40, price: 7.74 },
];

const GLS_5P = [
  { max: 1, price: 2.04 }, { max: 2, price: 2.04 }, { max: 3, price: 2.25 },
  { max: 5, price: 2.36 }, { max: 10, price: 3.08 }, { max: 15, price: 3.96 },
  { max: 20, price: 4.64 }, { max: 25, price: 5.38 }, { max: 30, price: 6.47 },
  { max: 40, price: 7.54 },
];

const DPD_TABLE = [
  { max: 1, price: 2.59 }, { max: 2, price: 2.59 }, { max: 3, price: 2.67 },
  { max: 5, price: 2.67 }, { max: 10, price: 3.25 }, { max: 15, price: 3.25 },
  { max: 20, price: 4.09 }, { max: 25, price: 4.09 }, { max: 31.5, price: 5.64 },
];
...
function volumeWeightInTime(items: Array<{ length: number; width: number; height: number }>) {
  return round2(items.reduce((sum, p) => sum + (p.length / 100) * (p.width / 100) * (p.height / 100) * 200, 0));
}
...
function inputStyle(): CSSProperties {
  return { padding: "14px 12px", fontSize: 17, border: "1px solid #d0d7de", borderRadius: 12, width: "100%", boxSizing: "border-box", fontFamily: "inherit", background: "#fff", minHeight: 50 };
}

function buttonStyle(primary = false): CSSProperties {
  return { padding: "12px 14px", borderRadius: 12, border: primary ? "1px solid #111827" : "1px solid #d1d5db", background: primary ? "#111827" : "#fff", color: primary ? "#fff" : "#111827", cursor: "pointer", fontWeight: 700, fontFamily: "inherit", minHeight: 46 };
}

function cardStyle(highlight = false): CSSProperties {
  return { border: highlight ? "2px solid #16a34a" : "1px solid #e5e7eb", background: highlight ? "#f0fdf4" : "#fff", borderRadius: 16, padding: 16, boxShadow: "0 1px 2px rgba(0,0,0,0.04)" };
}
...
function badgeStyle(type: "ok" | "warn" | "info" | "danger"): CSSProperties {
  ...
  return { display: "inline-block", padding: "5px 9px", borderRadius: 999, fontSize: 12, fontWeight: 800, ...styles[type] };
}

function serviceBadgeStyle(serviceType: ServiceType): CSSProperties {
  return { display: "inline-block", padding: "5px 10px", borderRadius: 999, fontSize: 12, fontWeight: 800, background: serviceType === "MBE Economy" ? "#16a34a" : "#dc2626", color: "#fff" };
}

function carrierPillStyle(name: string): CSSProperties {
  const base: CSSProperties = { display: "inline-flex", alignItems: "center", justifyContent: "center", padding: "6px 12px", borderRadius: 999, fontWeight: 800, fontSize: 14, minHeight: 34 };
...
function sectionSummaryStyle(): CSSProperties {
  return { cursor: "pointer", fontWeight: 800, listStyle: "none", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10 };
}
...
function calcHP(items: any[], cod: boolean): PriceResult {
  ...
  return { name: "HP", price: round2(price), possible: true, details: ["Max 30 kg po paketu", "Obračun po pošiljci", cod ? "COD +0,50 €" : "Bez COD dodatka", "Bez otočnih nadoplata"], serviceType: "MBE Economy", status: "ok" };
}

function calcGLS(items: any[], postal: string, cod: boolean): PriceResult {
  const reasons: string[] = [];
  ...
  return {
    name: "GLS", price: round2(price), possible: true, serviceType: "MBE Express", status: hasSurcharge ? "surcharge" : "ok",
    details: [items.length === 1 ? "1 paket cjenik" : items.length <= 4 ? "2–4 paketa cjenik" : "5+ paketa cjenik", "+19% dodatak", cod ? "COD +0,43 €" : "Bez COD dodatka", ...(oversize ? [`Prekomjerne dimenzije +${oversize.toFixed(2)} €`] : []), ...(overweight ? [`Prekomjerna težina +${overweight.toFixed(2)} €`] : [])],
    warning: warnings.length ? warnings.join(" · ") : undefined,
  };
}

function calcDPD(items: any[], postal: string): PriceResult {
  const reasons: string[] = [];
  const warnings: string[] = [];
  let oversize = 0;
  let overweight = 0;
  ...
  let price = items.length >= 2 ? items.length * 2.89 : tierPrice(DPD_TABLE, Math.min(items[0].weight, 31.5)) || 0;
  ...
  return { name: "DPD", price: round2(price), possible: true, serviceType: "MBE Economy", status: hasSurcharge ? "surcharge" : "ok", details, warning: warnings.length ? warnings.join(" · ") : undefined };
}

function calcOSSingle(items: any[], postal: string, cod: boolean): PriceResult {
  const reasons: string[] = [];
  const warnings: string[] = [];
  let base = 0, heavy = 0, bulky = 0;
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    if (p.weight > 31.5) { heavy += 4; warnings.push(`Paket ${i + 1}: teški paket +4,00 €`); }
    if (p.length > 100) { bulky += 4; warnings.push(`Paket ${i + 1}: glomazni paket +4,00 €`); }
  ...
  return {
    name: "Overseas Single", price: round2(price), possible: true, serviceType: "MBE Economy", status: hasSurcharge ? "surcharge" : "ok",
    details: ["Obračun po paketu", "+5% dodatak", cod ? "COD +0,30 €" : "Bez COD dodatka", isOverseasSpecial(postal) ? "Posebna zona / otok +20%" : "Standardna zona", ...(heavy ? [`Teški paket +${heavy.toFixed(2)} €`] : []), ...(bulky ? [`Glomazni paket +${bulky.toFixed(2)} €`] : [])],
    warning: warnings.length ? warnings.join(" · ") : undefined,
  };
}

function calcOSMulti(items: any[], postal: string, cod: boolean): PriceResult | null {
  ...
  return {
    name: "Overseas Multi", price: round2(price), possible: true, serviceType: "MBE Economy", status: hasSurcharge ? "surcharge" : "ok",
    details: ["Obračun po pošiljci", "+5% dodatak", cod ? "COD +0,30 €" : "Bez COD dodatka", isOverseasSpecial(postal) ? "Posebna zona / otok +20%" : "Standardna zona", ...(heavy ? [`Teški paket +${heavy.toFixed(2)} €`] : []), ...(bulky ? [`Glomazni paket +${bulky.toFixed(2)} €`] : [])],
    warning: warnings.length ? warnings.join(" · ") : undefined,
  };
}

function calcInTime(items: any[], postal: string): PriceResult {
  ...
  if (base === null) return fail("InTime", "MBE Economy", ["Obračunska masa prelazi 3000 kg"]);

  return {
    name: "InTime",
    price: round2(base * 1.2),
    possible: true,
    serviceType: "MBE Economy",
    status: "ok",
    details: [`Zona ${zone}`, `Stvarna masa ${actual} kg`, `Volumetrijska masa ${volumetric} kg`, `Obračunska masa ${chargeable} kg`, "+20% gorivo"],
  };
}
    ...
    <div style={cardStyle(highlighted)}>
      <div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
        <div style={{ minWidth: 0, flex: 1 }}>
          ...
          {result.warning ? <div style={{ marginTop: 10, color: "#9a3412", fontWeight: 700, lineHeight: 1.5 }}>PAZI: {result.warning}</div> : null}
        </div>
  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
      <div style={cardStyle(!!overallWinner)}>
        <div style={{ fontSize: 12, color: "#64748b", textTransform: "uppercase", fontWeight: 800 }}>Preporuka</div>
        {overallWinner ? (
          <>
            ...
            {overallWinner.warning ? <div style={{ marginTop: 8, color: "#9a3412", fontWeight: 700, lineHeight: 1.5 }}>PAZI: {overallWinner.warning}</div> : null}
          </>
        ) : <div style={{ color: "#64748b", marginTop: 8 }}>Upiši poštanski broj i paket.</div>}
      </div>

      <div style={cardStyle(false)}>
        <div style={{ fontSize: 12, color: "#64748b", textTransform: "uppercase", fontWeight: 800 }}>Express</div>
        {express ? (
          <>
            <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap", marginTop: 8 }}>
            ...
            {express.warning ? <div style={{ marginTop: 8, color: "#9a3412", fontWeight: 700, lineHeight: 1.5 }}>PAZI: {express.warning}</div> : null}
          </>
...
export default function App() {
  ...
  const isReady = useMemo(() => postalCode.length === 5 && packages.every((p) =>
  ...
  const results = useMemo(() => {
    ...
    const overallWinner = allPossible.length ? [...allPossible].sort((a, b) => (a.price ?? Infinity) - (b.price ?? Infinity))[0] : null;
  ...
  const reviewSurcharges = useMemo(() => {
    ...
    const surchargeDetails = [...results.economy, results.express]
      ...
      .flatMap((r) => r.details.filter((d) => d.includes("+") && /(dimenz|tež|teški|glomaz|otočna)/i.test(d)));
  ...
  const addPackage = () => setPackages((prev) => [...prev, { weight: "1", length: "10", width: "10", height: "10" }]);
  const duplicatePackage = (index: number) => setPackages((prev) => [...prev.slice(0, index + 1), { ...prev[index] }, ...prev.slice(index + 1)]);
  const removePackage = (index: number) => setPackages((prev) => prev.length === 1 ? prev : prev.filter((_, i) => i !== index));
  const resetShipment = () => { setPostalCode(""); setCod(false); setPackages([{ weight: "2", length: "30", width: "20", height: "10" }]); };
  const commonInputProps = { onFocus: (e: FocusEvent<HTMLInputElement>) => e.target.select() };
  const additionalPackages = packages.slice(1);

  return (
    <div style={{ minHeight: "100vh", background: "#f8fafc", padding: isMobile ? 12 : 16, paddingBottom: isMobile ? 140 : 16, fontFamily: "Ubuntu, Arial, sans-serif" }}>
      <div style={{ maxWidth: 980, margin: "0 auto", display: "grid", gap: 14 }}>
        <div style={{ ...cardStyle(), padding: isMobile ? 14 : 16 }}>
          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12 }}>
            <div>
              ...
              <div style={{ color: "#64748b", marginTop: 4, fontSize: isMobile ? 14 : 16 }}>Mail Boxes Etc. Križevci</div>
            </div>
            <button style={{ ...buttonStyle(), minHeight: 38, padding: isMobile ? "8px 12px" : "10px 14px", fontSize: isMobile ? 15 : 16 }} onClick={resetShipment}>Reset</button>
          </div>
        </div>
          ...
          <div style={cardStyle()}>
            <div style={{ display: "grid", gap: 10 }}>
              <div>
                ...
                <input {...commonInputProps} inputMode="numeric" value={postalCode} onChange={(e) => setPostalCode(e.target.value.replace(/\D/g, "").slice(0, 5))} placeholder="npr. 51550" style={inputStyle()} />
              </div>
              ...
              <div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr 1fr" : "repeat(4, minmax(0,1fr))", gap: 10 }}>
                  {(["weight", "length", "width", "height"] as const).map((field) => (
                    <div key={field}>
                      <label style={{ display: "block", marginBottom: 6 }}>{field === "weight" ? "Težina" : field === "length" ? "Duljina" : field === "width" ? "Širina" : "Visina"}</label>
                      <input {...commonInputProps} inputMode="decimal" type="text" value={packages[0]?.[field] ?? ""} onChange={(e) => updatePackage(0, field, e.target.value)} placeholder={field === "weight" ? "kg" : "cm"} style={inputStyle()} />
                    </div>
                  ))}
                </div>
              </div>
                  ...
                  <div key={index} style={{ ...cardStyle(), padding: 14 }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr 1fr" : "repeat(4, minmax(0,1fr))", gap: 10 }}>
                      {(["weight", "length", "width", "height"] as const).map((field) => (
                        <div key={field}>
                          <label style={{ display: "block", marginBottom: 6 }}>{field === "weight" ? "Težina" : field === "length" ? "Duljina" : field === "width" ? "Širina" : "Visina"}</label>
                          <input {...commonInputProps} inputMode="decimal" type="text" value={pkg[field]} onChange={(e) => updatePackage(index, field, e.target.value)} placeholder={field === "weight" ? "kg" : "cm"} style={inputStyle()} />
                        </div>
                      ))}
                    </div>
                  </div>
        ...
        <details style={cardStyle()} open>
          ...
          {results ? (
            <div style={{ marginTop: 14, display: "grid", gap: 12 }}>
              {results.economy.map((r) => <ResultRow key={r.name} result={r} highlighted={r.name === results.overallWinner?.name} />)}
            </div>
          ) : <div style={{ marginTop: 12, color: "#64748b" }}>Upiši poštanski broj i sve dimenzije paketa.</div>}
        </details>
      </div>

      <div style={{ position: isMobile ? "fixed" : "sticky", left: 0, right: 0, bottom: 0, zIndex: 30, padding: isMobile ? "10px 12px calc(10px + env(safe-area-inset-bottom))" : 0, background: isMobile ? "rgba(248,250,252,.96)" : "transparent", backdropFilter: isMobile ? "blur(10px)" : "none", borderTop: isMobile ? "1px solid #e5e7eb" : "none" }}>
        <div style={{ maxWidth: 980, margin: "0 auto" }}>
          <details style={cardStyle()}>
            ...
            <div style={{ marginTop: 12, display: "grid", gap: 8, color: "#475569" }}>
              ...
              <div>Nestandardno: <strong>{results ? [...results.economy, results.express].some((r) => r.status === "surcharge") ? "Da" : "Ne" : "-"}</strong></div>
              <div>Nadoplate: <strong>{results ? reviewSurcharges.length ? reviewSurcharges.join(" · ") : "Nema" : "-"}</strong></div>
              <div>Najpovoljnija opcija: <strong>{results?.overallWinner ? `${results.overallWinner.name} (${money(results.overallWinner.price)})` : "-"}</strong></div>
            </div>
          </details>
        </div>
      </div>
